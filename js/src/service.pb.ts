...
export async function ToUpper(
  toUpperRequest: model.ToUpperRequest,
  config?: ClientConfiguration,
): Promise<model.ToUpperResponse> {
  const response = await PBrequest(
    "/example.TestService2/ToUpper",
    model.ToUpperRequest.encode(toUpperRequest),
    config,
  );
  return model.ToUpperResponse.decode(response);
}
...
export async function ToUpperJSON(
  toUpperRequest: model.ToUpperRequest,
  config?: ClientConfiguration,
): Promise<model.ToUpperResponse> {
  const response = await JSONrequest(
    "/example.TestService2/ToUpper",
    model.ToUpperRequestJSON.encode(toUpperRequest),
    config,
  );
  return model.ToUpperResponseJSON.decode(response);
}
...
export interface TestService2<Context = unknown> {
  ToUpper: (
    toUpperRequest: model.ToUpperRequest,
    context: Context,
  ) => Promise<model.ToUpperResponse> | model.ToUpperResponse;
}

export function createTestService2<Context>(service: TestService2<Context>) {
  return {
    name: "example.TestService2",
    methods: {
      ToUpper: {
        name: "ToUpper",
        handler: service.ToUpper,
        input: {
          protobuf: model.ToUpperRequest,
          json: model.ToUpperRequestJSON,
        },
        output: {
          protobuf: model.ToUpperResponse,
          json: model.ToUpperResponseJSON,
        },
      },
    },
  } as const;
